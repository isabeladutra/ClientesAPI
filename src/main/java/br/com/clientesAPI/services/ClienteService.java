...
public class ClienteService {
	...
	public void insertCliente(Cliente cliente) throws ClienteExistenteException {
	    ...
	    // Insere o cliente e seus emails
	    ...
	    for (Email email : cliente.getEmails()) {
	        ...
	        entityManager.createNativeQuery(emailSql)
	            .setParameter("email", email.getEmail())
	            .setParameter("clienteId", insertedCliente.getId())
	            .executeUpdate();
	    }
	    
	    // Insere os celulares do cliente
	    for (Celular celular : cliente.getCelulares()) {
	        String celularSql = "INSERT INTO celular (numero, cliente_id) VALUES (:numero, :clienteId)";
	        entityManager.createNativeQuery(celularSql)
	            .setParameter("numero", celular.getNumero())
	            .setParameter("clienteId", insertedCliente.getId())
	            .executeUpdate();
	    }
	}
	...
	@Transactional
	public Cliente atualizarCliente(Long clienteId, Integer novoCpf, String novoNome, List<String> novosEmails, List<String> novosCelulares) {
	 ...
	 @Transactional
	    public void deleteClienteByCpf(Integer cpf) {
		 ...
		 Query deleteEmailQuery = entityManager.createNativeQuery("DELETE FROM email WHERE cliente_id IN (SELECT id FROM cliente WHERE cpf = :cpf)");
